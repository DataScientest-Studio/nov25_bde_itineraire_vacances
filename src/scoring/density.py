...
RESOLUTION_MAP = {"region": "h3_r6", "commune": "h3_r9"}
...
def add_density(lf: pl.LazyFrame, level: str = "commune") -> pl.LazyFrame:
    ...
    density = lf.group_by(h3_col).agg(
        pl.col("main_category_id").count().alias(density_col)
    )

    # 2) Normalisation Log
    density = (
        density.with_columns([(pl.col(density_col).log1p()).alias("density_log")])
        .with_columns(
            [
                (
