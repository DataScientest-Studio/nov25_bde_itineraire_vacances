...
def transform_datatourisme(file_path=None):
    ...
    df["h3_index"] = df.apply(
        lambda x: get_h3_index(x["latitude"], x["longitude"]), axis=1
    )

    # Préparation géométrie WKT pour PostGIS (POINT(lon lat))
    df["geom_wkt"] = df.apply(
        lambda x: f"POINT({x['longitude']} {x['latitude']})", axis=1
    )
    ...
    if "categories_raw" in df.columns:
        df.loc[
            df["categories_raw"].str.contains("Musée", case=False, na=False),
            "main_category",
        ] = "Culture"
        df.loc[
            df["categories_raw"].str.contains("Musée", case=False, na=False),
        ...
        df.loc[
            df["categories_raw"].str.contains("Parc", case=False, na=False),
            "main_category",
        ] = "Nature"
    ...
    df["popularity"] = [
        random.uniform(0, 500) for _ in range(len(df))
    ]  # Popularité brute
    ...
    # 8. Sauvegarde
    output_path = (
        PROCESSED_DIR / f"processed_vacances_{datetime.now().strftime('%Y%m%d')}.csv"
    )
