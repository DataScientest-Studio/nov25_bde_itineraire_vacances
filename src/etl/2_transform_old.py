...
KEYWORD_MAP = {
    ...
    'producteur': 10, 'ferme': 10,
    'brocante': 12, 'antiquite': 12,
    'restaurant': 13, 'auberge': 13,
    'marche': 14,
    'parc': 15, 'jardin': 15,
    'foret': 18, 'naturel': 18,
    'pique-nique': 19,
    'cheval': 20, 'equestre': 20,
    'bus touristique': 22, 'train touristique': 22,
    'zoo': 24, 'animal': 24,
    'spectacle': 25,
    'cafe': 26, 'bar': 26,
    'conference': 28,
    'nautique': 29, 'voile': 29,
    'paysage': 30, 'panorama': 30,
    'montagne': 31,
    'ouvrage': 33, 'pont': 33,
    'produit local': 34,
    'golf': 36,
    'musee': 37, 'expo': 37,
    'telepherique': 38,
    'cascade': 39, 'eau vive': 39,
    'aire de jeux': 40,
    'thermal': 42,
    'stade': 43, 'sport collectif': 43,
    'cinema': 44,
    'geologie': 47, 'grotte': 47,
    'sport mecanique': 48, 'karting': 48,
    'patrimoine civil': 49,
    'randonnee': 50, 'outdoor': 50,
    'concert': 51, 'musique': 51,
    'fete': 53, 'tradition': 53,
    'festival': 54,
    'soin': 55, 'bien-etre': 55,
    'foire': 57, 'salon': 57,
    'cimetiere': 58, 'memorial': 58,
    'ski': 61, 'hiver': 61,
    'thalasso': 62, 'balneo': 62,
    'accrobranche': 63, 'aventure': 63,
    'defile': 64, 'parade': 64,
    'vin': 65, 'spiritueux': 65, 'cave': 65
}
...
def extract_nested(row, keys):
    try:
        roots = [row.get('isLocatedAt'), row.get('hasLocation'), row.get('schema:address')]
        for root in roots:
            if not root: continue
            if isinstance(root, list) and len(root)>0: root=root[0]
            if isinstance(root, dict):
                addr = root.get('schema:address') or root.get('address') or root
                if isinstance(addr, list) and len(addr)>0: addr=addr[0]
...
def extract_geo(row, target):
    try:
        ...
        if isinstance(loc, dict):
            ...
            if isinstance(geo, list) and len(geo)>0: geo=geo[0]
...
def process_batch(data_list, first_batch):
    ...
    df_clean['nom_du_poi'] = nom.apply(lambda x: x[0] if isinstance(x, list) and len(x)>0 else x) if nom is not None else 'Inconnu'

    desc = get_col(['rdfs:comment.fr', 'Description'])
    df_clean['description'] = desc.apply(lambda x: str(x)[0:500] if isinstance(x, (str, list)) else '') if desc is not None else ''

    # 2. ADRESSE (Pour l'insertion dans la table adresse d'abord)
    df_clean['addr_rue'] = df.apply(lambda x: extract_nested(x, ['streetAddress']), axis=1).fillna('')
    df_clean['addr_cp'] = df.apply(lambda x: extract_nested(x, ['postalCode']), axis=1).fillna('')
    df_clean['addr_commune'] = df.apply(lambda x: extract_nested(x, ['addressLocality', 'locality']), axis=1).fillna('')

    # 3. GPS
    df_clean['latitude'] = pd.to_numeric(df.apply(lambda x: extract_geo(x, 'latitude'), axis=1), errors='coerce')
    df_clean['longitude'] = pd.to_numeric(df.apply(lambda x: extract_geo(x, 'longitude'), axis=1), errors='coerce')

    # 4. CATEGORIES
    cats = df_clean.apply(lambda x: determine_categories(x['nom_du_poi'], x['description']), axis=1)
    ...
    for r in [6, 7, 8, 9]:
        df_clean[f'h3_r{r}'] = df_clean.apply(lambda x: calculate_h3(x['latitude'], x['longitude'], r), axis=1)
    ...
    for s in scores: df_clean[s] = 0.0
...
def transform():
    if not os.path.exists(INPUT_FILE):
        print("❌ Pas de fichier source"); sys.exit(1)
        ...
        with zipfile.ZipFile(INPUT_FILE, 'r') as z:
            ...
            for i, f in enumerate(files):
                with z.open(f) as jf:
                    try: 
                        d = json.load(jf)
                        if isinstance(d, list): batch.extend(d)
                        else: batch.append(d)
                    except: continue
                if len(batch) >= BATCH_SIZE:
                    process_batch(batch, is_first)
                    batch = []; is_first = False
            if batch: process_batch(batch, is_first)
