            ...
            if isinstance(loc, dict):
                ...
                if addr:
                    ...
                    if isinstance(addr, dict):
                        street = (
                            addr.get("schema:streetAddress")
                            or addr.get("streetAddress")
                            or ""
                        )
                        cp = (
                            addr.get("schema:postalCode")
                            or addr.get("postalCode")
                            or ""
                        )
                        city = (
                            addr.get("schema:addressLocality")
        ...
        with open(target_path, "wb") as f:
            for i, chunk in enumerate(r.iter_content(chunk_size=1024 * 1024)):
                if chunk:
                    ...
                    if i % 10 == 0:
                        print(f"      ⏳ {(total_size/1024/1024):.1f} Mo...", flush=True)
    return total_size
        ...
        for region_name, url in URLS.items():
            ...
            try:
                ...
                if not zipfile.is_zipfile(TEMP_ZIP):
                    print("      ❌ Pas un ZIP valide", flush=True)
                    continue
                ...
                with zipfile.ZipFile(TEMP_ZIP, "r") as z:
                    json_files = [
                        f
                        for f in z.namelist()
                        if f.startswith("objects/") and f.endswith(".json")
                    ]
                    print(
                        f"      📦 {len(json_files)} fichiers à traiter...", flush=True
                    )
