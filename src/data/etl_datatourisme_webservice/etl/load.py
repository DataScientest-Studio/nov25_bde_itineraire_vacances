...
class DBManager:
    ...
    def insert_into_tables(
        ...
        for i, row in address_df.iterrows():
            self.cur.execute(
                "SELECT locality_id FROM localities WHERE locality_name = %s ",
                (row["locality"],),
            )
        ...
        for i, row in poi_df.iterrows():
            poi_row = address_df.loc[
                address_df["dc:identifier"] == row["dc:identifier"]
            ]
            poi_row = poi_row.iloc[0]
            self.cur.execute(
                "SELECT address_id FROM addresses WHERE (street_address = %s) AND (postal_code = %s) AND (locality = %s)",
                (
                    poi_row["street_adress"],
                    int(poi_row["postal_code"]),
                    poi_row["locality"],
                ),
            )
        ...
        for i, row in tel_df.iterrows():
            self.cur.execute(
                "SELECT poi_id FROM pois WHERE (poi_id_tourism = %s)",
                (row["dc:identifier"],),
            )
        ...
        for i, row in categories_df.iterrows():
            ...
            self.cur.execute(
                "SELECT category_id FROM categories WHERE (main_category = %s) AND (sub_category = %s) AND (itinerary_category = %s)",
                (row["main_category"], row["sub_category"], row["to_keep"]),
            )
