...
class DataTourismeExtractor:
    def __init__(self, feed_url, app_key, timeout=300, main_types=cf.main_types):
        self.feed_url = (
            feed_url  # URL du webservice créé sur la plateforme DataTourisme Diffuseur
        )
    ...
    def get_raw_poi_df(self):
        try:
            ...
            with ZipFile(io.BytesIO(response.content)) as zipfile:
                ...
                for file in index_df["file"]:
                    with zipfile.open("objects/" + file) as f:
                        data = json.load(f)
                    f_df = pd.json_normalize(data, errors="ignore", sep="_")
                    df_list.append(f_df)

            poi_df = pd.concat(df_list, axis=0)
            ...
            # Choix des structures de données à garder:
            columns = [
                "dc:identifier",
                "label",
                "@type",
    ...
    def get_general_df(self):
        if hasattr(self, "poi_df"):  # vérifie sur l'attribut poi_df existe :
            ...
            description_df = pd.json_normalize(data=self.poi_df["hasDescription"]).map(
                ext.simple_list_extract
            )
            description_df = description_df[["shortDescription.fr"]].rename(
                columns={"shortDescription.fr": "description"}
            )
            description_df.index = self.poi_df[["dc:identifier"]].index

            # extraction des données de contact des POI
            contact_df = pd.json_normalize(data=self.poi_df["hasContact"]).map(
                ext.simple_list_extract
            )
            ...
            general_df = pd.concat(
                [
                    self.poi_df[["dc:identifier", "lastUpdate", "label"]],
                    description_df,
                    contact_df,
                ],
                axis=1,
            ...
            print(
                "Vous devez créer l'attribut 'poi_df' avec la méthode 'get_raw_poi_df' pour pouvoir utiliser cette méthode "
            )
    ...
    def get_themes_df(self):
        ...
        if hasattr(self, "poi_df"):
            ...
            themes_df = themes_df[
                ["dc:identifier", "@type", "rdfs:label.fr"]
            ]  # récupération d'une partie des colonnes
            themes_df = themes_df.rename(
                columns={"@type": "theme", "rdfs:label.fr": "sub_theme"}
            )  # renomage des colonnes
            themes_df = themes_df.explode(
                "theme"
            )  # répartir les valeurs de thèmes sous forme de liste sur plusieurs lignes
    ...
    def get_location_df(self):
        ...
        if hasattr(self, "poi_df"):
            location_df = pd.json_normalize(
                data=self.poi_df["isLocatedAt"],
                max_level=1,
                meta=[
            ...
            new_columns_dict = {
                "adress_schema:addressLocality": "locality",
            ...
            location_df = location_df[columns_to_keep].rename(
                columns=new_columns_dict
            )  # choix des colonnes et renommage
            ...
            location_df = pd.concat(
                [self.poi_df[["dc:identifier"]], location_df], axis=1
            )
    ...
    def get_opening_hours_df(self):
        ...
        if hasattr(self, "poi_df"):
            ...
            is_located_at_df = pd.concat(
                [self.poi_df["dc:identifier"], is_located_at_df], axis=1
            )
            ...
            opening_hours_df = is_located_at_df[
                ["dc:identifier", "schema:openingHoursSpecification"]
            ].explode("schema:openingHoursSpecification")

            # répartition des données du dictionnaire 'schema: openingHoursSpecification' sur plusieurs colonnes
            schema_opening_hours_df = pd.json_normalize(
                data=opening_hours_df["schema:openingHoursSpecification"]
            )

            # suppression des colonnes de traduction des infos supplémentaires :
            schema_opening_hours_df = schema_opening_hours_df.drop(
                columns=[
                    "@type",
                    ...
                    "additionalInformation.nl",
                    "additionalInformation.es",
                ]
            )
            ...
            schema_opening_hours_df = schema_opening_hours_df.map(
                ext.simple_list_extract
            )
            ...
            opening_hours_df = pd.concat(
                [opening_hours_df[["dc:identifier"]], schema_opening_hours_df], axis=1
            )
        ...
        if hasattr(self, "poi_df"):
            ...
            reviews_df = reviews_df[
                [
                    "dc:identifier",
                    "hasReviewValue.@type",
                    "hasReviewValue.rdfs:label.fr",
                    "hasReviewValue.isCompliantWith",
                    "hasReviewValue.schema:ratingValue",
                ]
            ]
            # renommer les colonnes :
            rename_col_dict = {
                "dc:identifier": "poi_id",
    ...
    def get_offers_df(self):
        ...
        if hasattr(self, "poi_df"):
            ...
            offers_df_raw = offers_df_raw[
                ["dc:identifier", "schema:priceSpecification"]
            ]
            ...
            offers = pd.json_normalize(data=df["hasPricingOffer"]).map(
                ext.simple_list_extract
            )
            offers = offers[["@type", "rdfs:label.fr"]].rename(
                columns={
                    "@type": "pricing_offer_id",
                    "rdfs:label.fr": "pricing_offer_label",
                }
            )
            ...
            periods = periods[["startDate", "endDate"]].rename(
                columns={
                    "startDate": "pricing_start_date",
                    "endDate": "pricing_end_date",
                }
            )
            ...
            policies = policies[["@id", "rdfs:label.fr"]].rename(
                columns={
                    "@id": "pricing_policy_id",
                    "rdfs:label.fr": "pricing_policy_label",
                }
            )
            policies["pricing_policy_id"] = policies["pricing_policy_id"].str.replace(
                "kb:", ""
            )
            ...
            modes = modes[["@id", "rdfs:label.fr"]].rename(
                columns={
                    "@id": "pricing_mode_id",
                    "rdfs:label.fr": "pricing_mode_label",
                }
            )
            ...
            offers_df = pd.concat(
                [df[columns_to_keep], periods, offers, policies, modes], axis=1
            )
...
if __name__ == "__main__":
    ...
    print(
        level_1_types_df.groupby("type")["dc:identifier"]
        .count()
        .apply(lambda x: round(x / level_1_types_df.shape[0] * 100, 2))
    )
