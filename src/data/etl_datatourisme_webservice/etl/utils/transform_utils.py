...
from config.config import main_types, main_types_fr, new_categories_mapping_df
...
class ClassExtract:
    ...
    def get_main_types_df(self, main_types=main_types):
        ...
        classes_df["linked_parent_label"] = classes_df[
            "linked_parent_label"
        ].str.replace("<https://www.datatourisme.fr/ontology/core#", "")
        ...
        main_types_df = main_types_df.drop(
            columns=["parent_label", "linked_parent_label"]
        )
        main_types_df = main_types_df.rename(
...
def get_categories_mapping_df(
    classes_path=input_classes_fr_path,
    categories_mapping_path=input_categories_mapping_path,
    main_types=main_types_fr,
):
    """ "
    ...
    categories_mapping_df = pd.read_csv(input_categories_mapping_path).drop(
        columns="index"
    )
    ...
    df = pd.merge(
        main_types_df,
        categories_mapping_df,
        how="left",
        on=["level_1_label", "level_2_label", "level_3_label", "level_4_label"],
    )

    # ajout du linked_label associée à la colonne 'Level 3/4' :
    df.loc[df["Level_3_4"] == df["level_2_label"], "linked_label"] = df.loc[
        df["Level_3_4"] == df["level_2_label"], "level_2_linked_label"
    ]
    df.loc[df["Level_3_4"] == df["level_3_label"], "linked_label"] = df.loc[
        df["Level_3_4"] == df["level_3_label"], "level_3_linked_label"
    ]
    df.loc[df["Level_3_4"] == df["level_4_label"], "linked_label"] = df.loc[
        df["Level_3_4"] == df["level_4_label"], "level_4_linked_label"
    ]

    # ajout des nouvelles catégories issus de l'analyse des POIs
    df = pd.concat(
        [
            new_categories_mapping_df,
...
if __name__ == "__main__":
    ...
    print(
        f"la nouvelle matrice du mapping de catégories a été générée et enregistrée dans {INPUT_DIR/'last_mapping_matrix.csv'}, elle est contient {df.shape[0]} lignes"
    )
