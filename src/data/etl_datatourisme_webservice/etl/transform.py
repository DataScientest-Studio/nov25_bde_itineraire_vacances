...
def transform_types_df(types_df):
    ...
    categories_df = types_df.merge(
        categories_mapping_df, how="left", left_on="type", right_on="linked_label"
    )
    # filtrage des types de POI selon la valeur de 'to_keep'
    categories_df = categories_df[
        categories_df["to_keep"].isin([1, 2])
    ]  # filtrer sur 'to_keep' pour garder que les points de type 1 ou 2

    # sélection des colonnes nécessaires pour la construction de la table dans la BBD :
    categories_df = categories_df[
        ["dc:identifier", "main_category", "sub_category", "to_keep"]
    ]
    ...
    categories_df["to_keep"] = (
        categories_df["to_keep"] == 1
    )  # convertir les valeurs 1 en True et 2 en False pour l'inclusion du POI dans l'itinéraire ou pas.
...
def transform_general_location_df(general_df, location_df):
    ...
    email_df, tel_df, website_df = [
        df.explode(df.columns[-1]) for df in [email_df, tel_df, website_df]
    ]
    ...
    location_df.loc[:, non_null_string_columns] = location_df[
        non_null_string_columns
    ].astype(str)

    # formattage des colonnes numériques :
    numeric_columns = ["latitude", "longitude"]
    location_df.loc[:, numeric_columns] = location_df[numeric_columns].apply(
        pd.to_numeric, errors="coerce"
    )

    location_df.loc[:, "postal_code"] = (
    ...
    location_df.loc[:, "street_adress"] = location_df["street_adress"].apply(
        tu.adress_format
    )
    location_df.loc[:, "street_adress"] = (
        location_df["street_adress"].astype(str).replace("nan", "unknown")
    )
    ...
    location_df = location_df.merge(
        ref_idf_df, how="left", left_on="locality", right_on="nom_standard"
    )
    ...
    ref_row = (
        ref_idf_df[ref_idf_df["nom_standard"] == "Paris"].iloc[0].values
    )  # racupération des valeurs assignées à Paris
    ...
    poi_df = description_df.merge(
        location_df[["dc:identifier", "latitude", "longitude"]],
        how="left",
        on="dc:identifier",
    )
    ## Ajouts des catégorisations géographiques h3 des POI :
    poi_df["h3_level_5"] = poi_df.apply(
        lambda row: latlng_to_cell(row["latitude"], row["longitude"], 5), axis=1
    )
    poi_df["h3_level_7"] = poi_df.apply(
        lambda row: latlng_to_cell(row["latitude"], row["longitude"], 7), axis=1
    )
    poi_df["h3_level_9"] = poi_df.apply(
        lambda row: latlng_to_cell(row["latitude"], row["longitude"], 9), axis=1
    )
