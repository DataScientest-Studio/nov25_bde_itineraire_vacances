...
extractor = extract.DataTourismeExtractor(feed_url=feed_url, app_key=app_key)

# extraction des données brutes :
extractor.get_raw_poi_df()
now = datetime.now()
print(
    f"{now.strftime('%H:%M:%S')}: extraction du dataframe de données brutes des pois terminée"
)
...
now = datetime.now()
print(
    f"{now.strftime('%H:%M:%S')}: extraction des 3 dataframes general_df, location_df, types_df terminée, lancement de la transformation"
)
...
(
    ...
    h3_level_df,
    locality_df,
) = transform.transform_general_location_df(general_df, location_df)
now = datetime.now()
print(
    f"{now.strftime('%H:%M:%S')}: transformation des données terminée, lancement du chargement dans la base de données"
)
...
now = datetime.now()
print(
    f"{now.strftime('%H:%M:%S')}: Les données sont disponibles dans la base de donnée :)"
)
