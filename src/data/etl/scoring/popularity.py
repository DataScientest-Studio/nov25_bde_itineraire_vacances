...
def add_popularity(lf: pl.LazyFrame, k: int = 50) -> pl.LazyFrame:
    ...
    lf = lf.with_columns(
        [
            (
                ...
                if "rating" in lf.columns
                else pl.lit(0.0)
            ).alias("rating"),
            (
                pl.col("review_count").cast(pl.Float64).fill_null(0)
                if "review_count" in lf.columns
                else pl.lit(0.0)
            ).alias("review_count"),
        ]
    )
    ...
    # ---------------------------------------------------------
    lf = lf.with_columns([(pl.col("rating") / 5).clip(0, 1).alias("rating_norm")])
    ...
    lf = lf.with_columns(
        [
            ...
            .then(pl.col("review_count").map_elements(lambda r: 1 - math.exp(-r / k)))
            .otherwise(0.0)
            .alias("reviews_norm")
        ]
    )
    ...
    # ---------------------------------------------------------
    lf = lf.with_columns(
        [(pl.col("rating_norm") * pl.col("reviews_norm")).alias("popularity_norm")]
    )
