...
def poi_structure_extract(poi_df, data_struct):
    ...
    mask_simple_list = df_raw[data_struct].apply(
        lambda x: (isinstance(x, list)) and (len(x) == 1)
    )

    ## avec une structure hastheme sous forme d'une liste d'un seul élément
    mask_multiple_list = df_raw[data_struct].apply(
        lambda x: (isinstance(x, list)) and (len(x) > 1)
    )
    ...
    if len(simple_list_df) > 0:
        ...
        if mask_null.sum() > 0:
            ...
            df = pd.concat([df, df_raw[mask_null]], axis=0, ignore_index=True).map(
                simple_list_extract
            )
...
def poi_general_extract(poi_df):
    description_df = pd.json_normalize(data=poi_df["hasDescription"]).map(
        simple_list_extract
    )
    ...
    df = pd.concat(
        [
            poi_df[["dc:identifier", "label", "rdfs:comment_fr"]],
            description_df,
            contact_df,
        ],
        axis=1,
...
def poi_themes_extract(poi_df):
    ...
    ]  # récupération de certaines colonnes
...
def poi_location_extract(poi_df):
    ...
    location_df = pd.json_normalize(
        data=poi_df["isLocatedAt"],
        max_level=1,
        meta=[["schema:geo", "schema:latitude"], ["schema:geo", "schema:longitude"]],
        meta_prefix="_",
...
def poi_offers_extract(poi_df):
    ...
    offers = offers[["@type", "rdfs:label.fr"]].rename(
        columns={"@type": "pricing_offer_id", "rdfs:label.fr": "pricing_offer_label"}
    )
    ...
    periods = periods[["startDate", "endDate"]].rename(
        columns={"startDate": "pricing_start_date", "endDate": "pricing_end_date"}
    )
    ...
    policies = policies[["@id", "rdfs:label.fr"]].rename(
        columns={"@id": "pricing_policy_id", "rdfs:label.fr": "pricing_policy_label"}
    )
    ...
    modes = modes[["@id", "rdfs:label.fr"]].rename(
        columns={"@id": "pricing_mode_id", "rdfs:label.fr": "pricing_mode_label"}
    )
...
def classes_extract(path="classes_en.csv"):
    ...
    classes_df = pd.read_csv(
        path,
        skiprows=1,
        names=["linked_label", "label", "linked_parent_label", "parent_label"],
    )
    ...
    classes_df = classes_df[
        ["parent_label", "linked_parent_label", "label", "linked_label"]
    ]
    return classes_df
...
def types_tree(classes_df):
    ...
    main_types_df = main_types_df.merge(
        classes_df, how="left", left_on="label", right_on="parent_label"
    )
    main_types_df = main_types_df.drop(
        columns=["parent_label_y", "linked_parent_label_y"]
    )
    main_types_df = main_types_df.rename(
        columns={
            "parent_label_x": "level_1_label",
    ...
    main_types_df = main_types_df.merge(
        classes_df, how="left", left_on="level_3_label", right_on="parent_label"
    )
    main_types_df = main_types_df.drop(columns=["parent_label", "linked_parent_label"])
    main_types_df = main_types_df.rename(
        columns={"label": "level_4_label", "linked_label": "level_4_linked_label"}
    )
