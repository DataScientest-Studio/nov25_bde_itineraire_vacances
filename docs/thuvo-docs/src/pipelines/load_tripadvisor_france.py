...
PARQUET_PATH = (
    r"C:\Users\DELL\Downloads\ItineraireVacances3\df_tripadvisor_France.parquet"
)
...
def main():
    ...
    if "max_people" in df.columns:
        ...
        df.loc[
            (df["max_people"] < 0) | (df["max_people"] > 10000), "max_people"
        ] = pd.NA
    ...
    with psycopg2.connect(DSN) as conn:
        with conn.cursor() as cur:
            ...
            print(
                f"[TripAdvisor] inserted={inserted} updated={updated} history_rows_added={hist_added} total_now={total}"
            )
