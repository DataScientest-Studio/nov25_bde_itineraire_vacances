...
PARQUET_PATH = r"C:\Users\DELL\Downloads\ItineraireVacances3\df_prime_excursion.parquet"
DSN = "host=localhost port=5432 dbname=itineraire_vacances_prime user=thuvo password=admin"

TABLE = "silver.prime_excursion"
HIST_TABLE = "silver.prime_excursion_history"
STG = "_stg_prime_excursion"

# Colonnes dans l'ordre logique (mêmes noms que parquet)
BASE_COLS = [
    "source_id","poi_types","name","label_en","snippet","snippet_en","lat","lon","country",
    "region_insee","region","dept_insee","departement","city_insee","city","postal_code",
    "address_locality","address","theme_fr","theme_en","architectural_style_fr","architectural_style_en",
    "main_media_url","media_resource_url","url","rating","review_value_label_fr","review_value_label_en",
    "sub_category","opens_time","closes_time","hours_valid_from","hours_valid_through","max_people",
    "difficulty_level_fr","locomotion_mode_fr","tour_type_fr","tour_distance_m","duration_min",
    "practice_duration_min","duration_days","practice_duration_days","positive_elevation_gain_m",
    "negative_elevation_loss_m","start_date","end_date","last_update_datatourisme","price","min_price",
    "max_price","type_principal","is_resto_type","is_resto_label","is_resto","is_label_incontournable",
    "is_label_famille","is_label_handicap","is_label_hebergement","is_label_gastronomie",
    "is_label_artisanat","is_label_randonnee","is_label_green","is_etoile","price_level",
    "main_category_compressed","is_prime_plus","main_cat_weight","main_cat_candidates","format_label",
    "format_weight","tempo_label","tempo_weight","score_prime","source","review_count","distance_km"
]

# Colonnes qui sont "numériques" en cible, mais qu'on accepte en TEXT en staging (pour éviter CEDEX etc.)
CAST_INT_COLS = {"region_insee", "dept_insee", "city_insee", "max_people", "review_count"}
...
def main():
    ...
    for c in ["poi_types", "theme_fr", "theme_en", "type_principal", "source", "main_cat_candidates"]:
        if c in df.columns:
            df[c] = df[c].astype("string")
    ...
    with psycopg2.connect(DSN) as conn:
        with conn.cursor() as cur:
            ...
            cast_region_insee = "CASE WHEN s.region_insee ~ '^\\d+$' THEN s.region_insee::int END"
            cast_dept_insee   = "CASE WHEN s.dept_insee   ~ '^\\d+$' THEN s.dept_insee::int END"
            cast_city_insee   = "CASE WHEN s.city_insee   ~ '^\\d+$' THEN s.city_insee::int END"
            cast_max_people   = "CASE WHEN s.max_people   ~ '^\\d+$' THEN s.max_people::int END"
            cast_review_count = "CASE WHEN s.review_count ~ '^\\d+$' THEN s.review_count::int END"
            ...
            print(f"[Prime Classique] inserted={inserted} updated={updated} history_rows_added={hist_added} total_now={total}")
