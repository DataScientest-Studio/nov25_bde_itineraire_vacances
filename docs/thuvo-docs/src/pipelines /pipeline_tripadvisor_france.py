...
df_trip_clean[["excellent", "very_good", "average"]] = df_trip_clean[["excellent", "very_good", "average"]].fillna(0)

#les transformer en proportions (%) en basant sur la colonne "total_reviews_count"
df_trip_clean["excellent_pct"] = ((df_trip_clean["excellent"] / df_trip_clean["total_reviews_count"])*100).round(2)
df_trip_clean["very_good_pct"] = ((df_trip_clean["very_good"] / df_trip_clean["total_reviews_count"])*100).round(2)
df_trip_clean["average_pct"] = ((df_trip_clean["average"] / df_trip_clean["total_reviews_count"])*100).round(2)
...
#mapper les valeurs
price_mapping = {"€": 1, "€€-€€€": 2, "€€€€": 3}
df_trip_clean["price_category"] = (df_trip_clean["price_level"].map(price_mapping).fillna(1))
...
df_trip_clean["reco_score"] = (100 * (0.40 * df_trip_clean["Q"] + 0.25 * df_trip_clean["E"] + 0.20 * df_trip_clean["R"] + 0.15 * df_trip_clean["A"]) + (df_trip_clean["awards_binary"] * 3)).round(2)
...
cols_keep = ["restaurant_link", "restaurant_name", "country", "region", "city", "address", "latitude", "longitude", "postal_code",
             "avg_rating_filled", "total_reviews_count_filled", "url", "price_category", "reco_score",
             "vegetarian_friendly", "vegan_options", "gluten_free", "is_halal", "is_kosher","cuisine_continent", "cuisine_type"]
...
UX_COLUMNS = ["source_id", "source", "type", "url", "lat", "lon",
              ...
              "snippet", "rating", "review_count", "price_level", "reco_score",
